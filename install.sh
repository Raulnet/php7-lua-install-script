...
for PKG in  php7-dev libncurses7-dev php-pear libreadline-dev make; do
...
LUA_VERSION=5.3.4
LUA_PECL_VERSION=2.0.5
LUA_GET_URL=${LUA_DOWNLOAD_URL}lua-${LUA_VERSION}.tar.gz
PHP_LUA_DOWNLOAD_URL=http://pecl.php.net/get/lua-${LUA_PECL_VERSION}.tgz
PHP_VERSION=$(grep '#define PHP_API_VERSION' /usr/include/php7/main/php.h|sed 's/#define PHP_API_VERSION//' | sed 's/\s//');
...
tar -xf lua-${LUA_PECL_VERSION}.tgz
...
cd /tmp/phplua/lua-${LUA_PECL_VERSION}
echo "Patching config for building in 64bit system"
patch -p1 ${SCRIPT_DIR}/config.m4 -i ${SCRIPT_DIR}/config_x64.path
cp ${SCRIPT_DIR}/config.m4 /tmp/phplua/lua-${LUA_PECL_VERSION}/
phpize
./configure
# patching php-lua by adding include path for lua sources
sed -i "s/INCLUDES =/INCLUDES = -I\/tmp\/phplua\/lua-${LUA_VERSION}\/src/g" /tmp/phplua/lua-${LUA_PECL_VERSION}/Makefile
make
make install
